...
This is an implementation of Zero-DP from https://arxiv.org/abs/1910.02054

It keeps shards of the optimizer state, gradients and parameters into multiple devices/nodes.

Memory consumption is reduced to ((2+2+K)*Psi)/Nd, where:
- 2 bytes per fp16 parameter
- 2 bytes per fp16 gradient
- K bytes for Adam (4 bytes for parameter, first moment, second moment - all in fp32)
- Psi is total number of parameters
- Nd is total number of devices (each device holds one shard)s

Communication volume is O(3*Psi) vs standard data-parallel training O(2*Psi)

This only implements Zero-DP, and not Zero-R memory optimizations.

There's a script to fine-tune GPT NeoX using Zero-DP memory optimization.
"""
...
class Zero3Layer(nn.Module):
    ...
    # chunk[0] is for trainable parameters and chunk[1] is for fixed parameters.
    chunk: List[nn.Parameter]
    # Sizes of the chunks in chunk list.
    chunk_size: List[int]
    # First chunk is for trainable parameters.
    ...
    def __init__(self, module: nn.Module, rank: int, world_size: int, device: torch.device, dtype: torch.dtype):
        ...
        with torch.no_grad():
            ...
            # Empty tensor to receive the trainable and fixed parameters combined
    ...
    def forward(self, *args, **kwargs):
        ...
        # Start fetching parameters of the proceeding layers, so that they will fetch them while the current layer does its computations.
